...
function Example(props) {
  ...
  async function MakeTransaction(device) {
    ...
    const api = new ethers.providers.EtherscanProvider("rinkeby");
  ...
  async function MakeTransaction(device) {
    ...
    const { commandData, rawTx } = SecuxETH.ERC1155.prepareSafeTransferFrom(
      ...
      },
      {
        ...
        id: "<Token Id>",
        value: "<Amount>",
        // give a value if needed
        // data: "<Arbitrary Hex String>"
      }
    );
    const response = await device.Exchange(commandData);
    const signedTx = SecuxETH.resolveTransaction(response, rawTx);
...
function Example(props) {
  ...
  async function MakeTransaction(device) {
    const path = "m/44'/60'/0'/0/0";
    const tx = Buffer.alloc();
    const commandData = SecuxETH.prepareSignSerialized(
      path,
      tx
    );
    const response = await device.Exchange(commandData);
